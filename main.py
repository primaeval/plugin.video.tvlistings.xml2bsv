...
def xml_channels():
    if plugin.get_setting('bsv_reload') == 'true':
        plugin.set_setting('bsv_reload','false')
    else:
        path = xbmc.translatePath(plugin.get_setting('bsv_folder'))
    ...
    xbmcvfs.mkdir('special://userdata/addon_data/plugin.video.tvlistings.xml2bsv')
    if not xbmcvfs.exists('special://userdata/addon_data/plugin.video.tvlistings.xml2bsv/myaddons.ini'):
        f = xbmcvfs.File('special://userdata/addon_data/plugin.video.tvlistings.xml2bsv/myaddons.ini','w')
        f.close()

    file_name = 'special://userdata/addon_data/plugin.video.tvlistings.xml2bsv/template.ini'
    ...
    for channel_id in channels:
        ...
        for total_seconds in programmes:
            ...
            if match:
                ...
                item = {'label':label,'icon':img_url,'thumbnail':img_url}
                item['info'] = {'plot':plot, 'season':int(season), 'episode':int(episode), 'genre':categories}
                item['path'] = plugin.url_for('play', channel_id=channel_id, channel_name=channel_name, title=title, season=season, episode=episode)
                items.append(item)
    plugin.set_view_mode(51)
    plugin.set_content('episodes')
    return items 
